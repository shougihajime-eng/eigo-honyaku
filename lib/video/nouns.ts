...
function dictMatch(jp: string): string | null {
...
- "low"   自信なし（en は空文字 "" にしてユーザー確認に回す）`;
  ...
  // 辞書ヒットを優先し、AI の confidence を source に変換
  const out: ExtractedNoun[] = [];
  const seen = new Set<string>();
  for (const n of parsed.nouns) {
    if (!n.jp || seen.has(n.jp)) continue;
    seen.add(n.jp);
    const dictEn = dictMatch(n.jp);
    if (dictEn) {
      out.push({ jp: n.jp, category: n.category, en: dictEn, source: "dictionary" });
    } else if (n.en && n.confidence === "high") {
      out.push({ jp: n.jp, category: n.category, en: n.en, source: "ai-confident" });
    } else if (n.en) {
      out.push({ jp: n.jp, category: n.category, en: n.en, source: "ai-uncertain" });
    } else {
